...
def getcal(request):
	if request.method=='POST':
		file=request.POST['file']
		gram=int(request.POST['gram'])
		print(file, gram)
		
		tagetimage='C:\\Users\\14694\\Desktop\\django\\FoodCalorie\\Testdata\\'+file
		from .FOODCNN import CNN

		name=CNN.detect(tagetimage)

		d=dataset.objects.filter(name__exact=name)
		cal=0.0
		for d1 in d:
			print(d1.calorie)
			cal=float(d1.calorie)
		cal=cal*gram
		cal=int(cal)
